# Week 5 · Evals & observability

Grading the path, not just the answer, and seeing what the system actually did
in production.

| # | Folder | What it is |
|---|--------|-----------|
| S9 | [`s09-trajectory-evals`](s09-trajectory-evals) | What to grade (`real-world`), who grades it (`judge`), when it blocks a release (`suite`) |
| S10 | [`s10-harness-tracing`](s10-harness-tracing) | The span recorder (`tracing`), the three numbers and their alert lines (`dashboards`), content-hashed prompt ids (`versioning`), routing to a human (`hitl`) |
| Lab 5 | [`lab-eval-harness`](lab-eval-harness) | Golden set, graders, trajectory check, pass-rate report that catches a planted regression |

Plant the regression yourself. A harness you have never seen fail is not
evidence that it works.
